...
CameraInputListenerSystem::~CameraInputListenerSystem() {
    for (auto id:subscribeIds) {
        EventDispatcher::getInstance().unsubscribe(id);
    }
}
void CameraInputListenerSystem::onStart() {
    size_t id=EventDispatcher::getInstance().subscribe("mouse wheeled",[this](std::any param) {
        ...
        //通常上滑滚轮是为了放大，即缩小视口
        cameraComp.captureWidth-=eventInfo.y*100;
        cameraComp.captureHeight-=eventInfo.y*100;
    ...
    id=EventDispatcher::getInstance().subscribe("key w",[this](std::any param) {
    ...
    subscribeIds.push_back(id);
    id=EventDispatcher::getInstance().subscribe("key s",[this](std::any param) {
        assert(ecs::getComponent<TransformComp>(scene,curEntity).has_value());
        auto transformComp = ecs::getComponent<TransformComp>(scene,curEntity).value();
    ...
    subscribeIds.push_back(id);
    id=EventDispatcher::getInstance().subscribe("key a",[this](std::any param) {
        assert(ecs::getComponent<TransformComp>(scene,curEntity).has_value());
        auto transformComp = ecs::getComponent<TransformComp>(scene,curEntity).value();
    ...
    subscribeIds.push_back(id);
    id=EventDispatcher::getInstance().subscribe("key d",[this](std::any param) {
        assert(ecs::getComponent<TransformComp>(scene,curEntity).has_value());
        auto transformComp = ecs::getComponent<TransformComp>(scene,curEntity).value();
    ...
    subscribeIds.push_back(id);
    id=EventDispatcher::getInstance().subscribe("key q",[this](std::any param) {
        auto transformComp = ecs::getComponent<TransformComp>(scene,curEntity).value();
        transformComp.transform.rotation.angle-=10;//逆时针
        ecs::setComponent<TransformComp>(scene,curEntity, transformComp);
    });
    subscribeIds.push_back(id);
    id=EventDispatcher::getInstance().subscribe("key e",[this](std::any param) {
        auto transformComp = ecs::getComponent<TransformComp>(scene,curEntity).value();
